...
最低等级需求：95  
入场费用：1 Tol Rune  
前置任务：[The Breaking Point (level 97)](https://wynncraft.wiki.gg/wiki/The_Breaking_Point)

Tcc是当前游戏流程内最难的raid之一（吗？

需要注意的是，tcc的前置和前置的前置和前置的前置的前置很多，建议有机会就尽快完成，以免后面补任务坐牢
...
需注意，平台周围会持续刷怪，且平台会随机启动防御模式击飞平台上的玩家，请保护好自己。上下两个平台被同时踩住时，平台的防御模式会被关闭

当上方倒计时结束后，必须要站在正中间1格，才能帮助下方停止防御模式

所以上方倒计时结束时，守点玩家应该继续站台直到下方计时结束
...
miniboss伤害较高但血量一般，推荐灌伤职业站台秒杀miniboss
...
![](/assets/img/advanced/raid/tcc2_labyrinth.jpg)

当上帝视角的玩家找到终点，其需要为其他队友引路，迷宫内会有多个封闭的门，其余玩家需要击杀守门的miniboss才能继续前进。

本关开始时，由一名战斗能力较弱玩家选择拿取翅膀，其余玩家全部进入蓝色区域开路
...
当两个石傀儡都被护送至各自的平台且未被摧毁时，房间即成功完成，任意Golem被摧毁则Raid失败
...
![](/assets/img/advanced/raid/tcc2_golem.jpg)
...
这个房间中玩家需要完成四个任务解锁四个封印符文，同时保护Astral

![](/assets/img/advanced/raid/tcc3.jpg)
...
当陨石未成功防御时，Astal会展开保护罩，在接下来的短时间内免疫陨石的攻击

@tab Miniboss: Primeval Machine
...
当其生命值降至50%时，会召唤数个Potassium Axons，同时半血后会开始释放高频高伤害的弹幕
...
一般来说，四名玩家可以分散开完成四个挑战，前往Machine的玩家最好为DPS角色，在速杀完成后立即返回def进行防御，在这之前陨石会砸到一两次，不要紧
...
## Boss: The Canyon Colossus

tcc的场地是一系列碎片，掉出碎片会被弹起并受到高额伤害

tcc的平a包括直接用拳头或者用剑打出大范围超高伤害

### 技能列表

| 技能   | 触发条件     | 描述                                                                            |
|:-----|:---------|:------------------------------------------------------------------------------|
| 巨剑横扫 |          | 峡谷巨像会先收回剑，然后挥剑扫过整个场地。位于巨像剑道轨迹上的碎片会显示为红色。安全区在场地最后面那几块                          |
| 泰坦头槌 |          | 当玩家处于范围内时，峡谷巨像会用头撞击所有靠近的玩家，造成极大击退效果                                           |
| 剑刃猛击 |          | 峡谷巨像会击中场地中的部分平台，使其沉入地面。碎片将在20秒后重新出现。该技能发动后，需至少冷却20秒才能再次使用                     |
| 毁灭视线 | 生命值降至50% | 峡谷巨像会从眼中向场地远端的一个平台发射绿色激光，永久性地摧毁该平台直至战斗结束                                      |
| 灭绝之眼 |          | 峡谷巨像的眼睛将变为红色，随后向玩家丢陨石                                                         |
| 蓝焰   |          | 峡谷巨像的眼睛变为蓝色并举起剑。玩家当前位置下方会出现蓝色符文，随后标记处会升起净化光束                                  |
| 地裂之刃 | 生命值降至50% | 巨像举起右手召唤场边嵌着的一把剑。剑落下攻击前会有红色符文作为预警。剑击中场地时，会对范围内的玩家造成巨大伤害，并产生向外扩散的冲击波(notg同款白光) |

### 技巧

首先不要怕这些技能，看起来很吓人实际上有几个基本上没机会放出来(比如地裂之刃)

战斗一开始，tcc会固定释放一个平a，攻击范围覆盖中间和右侧的平台，所以一开始直接去左平台打。战斗过程中注意tcc的动作，有异常就跑就行了

还有，tcc的碰撞箱很大，紧贴最前方平台，如果有3block gambit的话要注意距离

除此之外，打桩灌伤就行

## Graid

巨像技能的前摇缩短，技能频率显著增加，巨剑横扫的范围会更大，地裂之刃会在战斗开始时激活。

巨像两侧会出现两座火山。当其中一座火山激活时，屏幕中央会有一个图标。南边的火山会释放大量小型陨石，覆盖范围更广；北边的火山则会发射数量较少但体积更大的陨石，如果玩家被命中，将受到大量伤害

陨石落地后会破裂成小陨石造成二次伤害

目前而言，gtcc只能通过灌伤在约30s内击杀，否则一旦地裂之刃、火山的前摇结束，队伍将完全失去输出能力(全都在躲技能)，所以对dps要求非常高